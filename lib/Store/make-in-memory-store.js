...
const makeInMemoryStore = (config) => {
    ...
    const contactHashIndex = new Map()
    const registerContactHash = (contactId) => {
        if (contactHashCache.has(contactId)) return
        const { user } = WABinary_1.jidDecode(contactId)
        const hash = Utils_1.md5(Buffer.from(user + 'WA_ADD_NOTIF', 'utf8')).toString('base64').slice(0, 3)
        contactHashCache.set(contactId, hash)
        if (!contactHashIndex.has(hash)) {
            contactHashIndex.set(hash, contactId)
        }
    }
    const unregisterContactHash = (contactId) => {
        const hash = contactHashCache.get(contactId)
        contactHashCache.delete(contactId)
        if (hash && contactHashIndex.get(hash) === contactId) {
            contactHashIndex.delete(hash)
        }
    }
    const contactsUpsert = (newContacts) => {
        ...
        for (const contact of newContacts) {
            ...
            registerContactHash(contact.id)
        }
        return oldContacts
    }
        ...
        ev.on('contacts.update', async (updates) => {
            for (const update of updates) {
                ...
                if (contacts[update.id]) {
                    ...
                    const matchedId = contactHashIndex.get(update.id?.[0])
                    contact = matchedId ? contacts[matchedId] : undefined
                }
                if (contact) {
