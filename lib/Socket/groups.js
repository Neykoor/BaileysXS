...
const extractGroupMetadata = (result) => {
    ...
    if (!group) {
        ...
        if (errorNode) {
            ...
            throw new (require("@hapi/boom").Boom)(text, { statusCode: code, data: errorNode })
        }
        throw new (require("@hapi/boom").Boom)('Invalid group metadata response: missing node', { data: result })
    }

    if (!group.attrs.id) {
        throw new (require("@hapi/boom").Boom)('Invalid group metadata response: missing group id', { data: group })
    }
    ...
    const metadata = {
        ...
        participants: WABinary_1.getBinaryNodeChildren(group, 'participant').map(({ attrs }) => {
            return {
                ...
                phoneNumber: WABinary_1.isLidUser?.(attrs.jid) && attrs.phone_number ? attrs.phone_number : undefined,
