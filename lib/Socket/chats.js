...
const makeChatsSocket = (config) => {
    ...
    /**
     * Mutex used to serialize per-chat processing of inbound stanzas.
     * Using a keyed mutex (by chat JID) lets messages from different chats be
     * processed in parallel instead of blocking on a single global queue.
     * Calls without an explicit key fall back to a global slot for compatibility.
     */
    const keyedProcessingMutex = make_mutex_1.makeKeyedMutex()
    const processingMutex = {
        mutex(keyOrFn, maybeFn) {
            if (typeof keyOrFn === 'function') {
                return keyedProcessingMutex.mutex('__global__', keyOrFn)
            }
            return keyedProcessingMutex.mutex(keyOrFn || '__global__', maybeFn)
        }
    }
