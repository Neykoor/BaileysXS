...
const makeMessagesRecvSocket = (config) => {
    ...
    // Persist cache across reconnects and allow fallback
    const group_cache = global.groupMetadataCacheMap || {}
    global.groupMetadataCacheMap = group_cache
    ...
    global.groupMetadataCache = async (jid) => {
        ...
        try {
            ...
            const lid_mapping = data.participants
                .filter(item => item.id && item.lid)
                .map(item => ({
                    pn: item.id,
                    lid: item.lid
                }));
            if (lid_mapping.length > 0) {
                await signalRepository.lidMapping.storeLIDPNMappings(lid_mapping)
            }
            return data
        } catch (error) {
            logger.warn({ jid, error: error?.message }, 'failed to fetch group metadata, using cache fallback')
            if (group_cache[jid]) {
                return group_cache[jid].data
            }
            return undefined
        }
    }
    ...
    const handleNewsletterNotification = (id, node) => {
        ...
        if (reactionsList) {
            const reactions = WABinary_1.getBinaryNodeChildren(reactionsList, 'reaction')
        ...
        } else if (viewsList?.attrs?.count) {
            ev.emit('newsletter.view', {
                id,
                newsletter_server_id: serverId,
                count: +viewsList.attrs.count
            })
        }
    }
    ...
    const handleMexNotification = (id, node) => {
        ...
        // === Reachout Timelock notification (from official Baileys) ===
        if (operation === 'NotificationReachoutTimelockUpdate') {
            const payload = content?.data?.xwa2_notify_reachout_timelock_info_update
            if (payload) {
                const enforcementType = payload.enforcement_type ?? 'DEFAULT'
                const timeEnforcementEnds =
                    payload.time_enforcement_ends && payload.time_enforcement_ends !== '0'
        ...
        // === Message Capping notification (from official Baileys) ===
        if (operation === 'NotificationNewChatMessagesCappingInfoUpdate') {
            const payload = content?.data?.xwa2_notify_new_chat_messages_capping_info_update
            if (payload) {
    ...
    const handleReceipt = async (node) => {
        ...
        try {
            await Promise.all([
                processingMutex.mutex(remoteJid || '__receipt__', async () => {
    ...
    const handleNotification = async (node) => {
        ...
        try {
            await Promise.all([
                processingMutex.mutex(remoteJid || '__notification__', async () => {
    ...
    const handleMessage = async (node) => {
        ...
        try {
            decryptionResult = Utils_1.decryptMessageNode(node, authState.creds.me.id, authState.creds.me.lid || '', signalRepository, logger)
        } catch (err) {
            // catch simple decryption errors to prevent message loss
            const key = {
            ...
            decryptionResult = {
                fullMessage: {
                    ...
                    messageStubType: WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT,
                    messageStubParameters: [err.message]
                },
                category: 'peer',
                author: node.attrs.participant || node.attrs.from,
                decrypt: async () => { }
            }
        }
        ...
        try {
            await Promise.all([
                processingMutex.mutex(msg?.key?.remoteJid || node.attrs.from || '__msg__', async () => {
                    await decrypt()
                    if (msg.messageStubType === WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT) {
                        ...
                        // Fire receipts in background - they don't need to block message dispatch
                        sendReceipt(msg.key.remoteJid, participant, [msg.key.id], type)
                            .catch(err => logger.warn({ err }, 'failed to send receipt'))

                        const isAnyHistoryMsg = Utils_1.getHistoryMsg(msg.message)
                        if (isAnyHistoryMsg) {
                            const jid = WABinary_1.jidNormalizedUser(msg.key.remoteJid)
                            sendReceipt(jid, undefined, [msg.key.id], 'hist_sync')
                                .catch(err => logger.warn({ err }, 'failed to send hist_sync receipt'))
                        }
                    }
                    Utils_1.cleanMessage(msg, authState.creds.me.id)
                    sendMessageAck(node).catch(err => logger.warn({ err }, 'failed to send message ack'))
