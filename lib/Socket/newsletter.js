...
const makeNewsletterSocket = (config) => {
    ...
    return {
        ...
        newsletterFetchAllParticipating: async () => {
        	...
        	const jidsToFetch = newsletters.filter(i => WABinary_1.isJidNewsletter(i.id)).map(i => i.id)
        	const metadataList = await Promise.all(jidsToFetch.map(jid => newsletterMetadata('JID', jid)))
        ...
        newsletterAdminCount: async (jid) => {
            ...
            const parsed = JSON.parse(WABinary_1.getBinaryNodeChild(result, 'result')?.content?.toString())
            return parsed?.data?.xwa2_newsletter_admin_count?.admin_count || 0
        },
        newsletterSubscribers: async (jid) => {
            const result = await newsletterWMexQuery(jid, Types_1.QueryIds.SUBSCRIBERS)
            const parsed = JSON.parse(WABinary_1.getBinaryNodeChild(result, 'result')?.content?.toString())
            return parsed?.data?.xwa2_newsletter_subscribers?.subscribers || 0
        },
