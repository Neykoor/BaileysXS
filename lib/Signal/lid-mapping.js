...
class LIDMappingStore {
    ...
    async getLIDsForPNs(pns) {
        ...
        for (const pn of pns) {
            ...
            if (lid) pairs.push({ pn, lid })
        }
        return pairs.length ? pairs : null
    }

    async getLIDForPN(pn) {
        if (!WABinary_1.isJidUser(pn) && !WABinary_1.isHostedPnUser(pn)) return null
        ...
        if (!lidUser) {
            ...
            if (lidUser) {
                ...
                let lookupPn = pn
                if (WABinary_1.isHostedPnUser(pn)) {
                    lookupPn = `${pnUser}@s.whatsapp.net`
                }
                const onWaResult = (await this.onWhatsAppFunc?.(lookupPn))?.[0]
                const exists = onWaResult?.exists
                const lid = onWaResult?.lid
                if (exists && lid) {
        ...
        const deviceSpecificLid = `${lidUser}:${pnDevice}@${lidServer}`
    ...
    async getPNForLID(lid) {
        if (!WABinary_1.isLidUser(lid) && !WABinary_1.isHostedLidUser(lid)) return null
        ...
        const pnJid = `${pnUser}:${lidDevice}@${pnServer}`
