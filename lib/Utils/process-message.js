...
const isRealMessage = (message, meId, normalizedContent) => {
    const normalizedContentResolved = normalizedContent !== undefined ? normalizedContent : messages_1.normalizeMessageContent(message.message)
    const hasSomeContent = !!messages_1.getContentType(normalizedContentResolved)
    return (
        !!normalizedContentResolved ||
    ...
    hasSomeContent &&
    !normalizedContentResolved?.protocolMessage &&
    !normalizedContentResolved?.reactionMessage &&
    !normalizedContentResolved?.pollUpdateMessage
}
...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, signalRepository, keyStore, logger, options, getMessage }) => {
    ...
    const content = messages_1.normalizeMessageContent(message.message)
    const isRealMsg = isRealMessage(message, meId, content)
    if (isRealMsg) {
