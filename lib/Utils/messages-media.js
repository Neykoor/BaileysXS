...
const getImageProcessingLibrary = exports.getImageProcessingLibrary = async () => {
    const tryImport = mod => Promise.resolve().then(() => __importStar(require(mod))).catch(() => undefined)
    const [_jimp, sharp, image] = await Promise.all([tryImport('jimp'), tryImport('sharp'), tryImport('@napi-rs/image')])
    if (sharp) return { sharp }
    if (image?.Transformer) return { image }
...
const prepareStream = async (media, mediaType, { logger, saveOriginalFileIfRequired, opts } = {}) => {
    ...
    try {
        for await (const data of stream) {
            ...
            sha256.update(data)
            if (!encFileWriteStream.write(data)) {
                await events_1.once(encFileWriteStream, 'drain')
            }
