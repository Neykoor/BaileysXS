...
const serializeMessage = async (msg, repository) => {
    ...
    if (lidsToResolve.length > 0) {
        ...
        const mentionReplacements = new Map()
        ...
        for (const result of results) {
            ...
            if (result.type === 'participant') {
                ...
                mentionReplacements.set(lidUser, pnUser)
            }
        }

        
        if (mentionReplacements.size > 0) {
            const target = msg[mtype]
            const field = target?.text ? 'text' : target?.caption ? 'caption' : null
            if (field) {
                const pattern = new RegExp('@(' + [...mentionReplacements.keys()].join('|') + ')', 'g')
                target[field] = target[field].replace(pattern, (_match, lidUser) => `@${mentionReplacements.get(lidUser)}`)
            }
        }
    }
