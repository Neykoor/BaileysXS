...
const decodeSyncdMutations = async (msgMutations, initialState, getAppStateSyncKey, onMutation, validateMacs) => {
    ...
    async function getKey(keyId) {
        ...
        if (keyCache.has(base64Key)) {
            return keyCache.get(base64Key)
        }
        const keyEnc = await getAppStateSyncKey(base64Key)
        if (!keyEnc) {
            throw new boom_1.Boom(`failed to find key "${base64Key}" to decode mutation`, { statusCode: 404, data: { msgMutations } })
        }
        const derived = await mutationKeys(keyEnc.keyData)
        keyCache.set(base64Key, derived)
        return derived
    }
}
...
const decodePatches = async (name, syncds, initial, getAppStateSyncKey, options, minimumVersionNumber, logger, validateMacs = true) => {
    ...
    for (const syncd of syncds) {
        ...
        if (validateMacs) {
            ...
            let result = keyCache.get(base64Key)
            if (!result) {
                const keyEnc = await getAppStateSyncKey(base64Key)
                if (!keyEnc) {
                    throw new boom_1.Boom(`failed to find key "${base64Key}" to decode mutation`)
                }
                result = await mutationKeys(keyEnc.keyData)
                keyCache.set(base64Key, result)
            }
            const computedSnapshotMac = generateSnapshotMac(newState.hash, newState.version, name, result.snapshotMacKey)
