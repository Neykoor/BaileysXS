...
/**
 * companion_platform_id uses its own enum, NOT DeviceProps.PlatformType.
 * Sending the wrong one makes the pairing-code flow fail on WhatsApp's side.
 */
...
/** QR payload format currently emitted by WA Web */
...
const generateRegistrationId = () => {
    return (Uint16Array.from(crypto_1.randomBytes(2))[0] & 16383) || 1
}
...
const generateMessageID = (userJid) => {
    const prefix = userJid ? 'RYZE' : 'BAE5'
    return prefix + crypto_1.randomBytes(12).toString('hex').toUpperCase()
}

const generateMessageIDV2 = (userJid) => {
    if (!userJid) {
        return 'RYZE' + crypto_1.randomBytes(12).toString('hex').toUpperCase()
    }
    const encoded = Buffer.from(userJid.replace(/@.+/, ''), 'utf-8')
    const hash = crypto_2.sha256(encoded).slice(0, 4)
    const random = crypto_1.randomBytes(8)
    return '3EB0' + Buffer.concat([hash, random]).toString('hex').toUpperCase()
}
...
const VERSION_SOURCES = [
    ...
    },
    {
        ...
        // scan the whole file: the declaration has moved lines several times upstream
        parse: data => {
            const m = String(data).match(/const\s+version\s*=\s*\[\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\]/)
            return m && [+m[1], +m[2], +m[3]]
        }
    },
    {
        // last resort: read it straight off WA Web itself
        url: 'https://web.whatsapp.com/sw.js',
        parse: data => {
            const m = String(data).match(/\\?"client_revision\\?"\s*:\s*(\d+)/)
            return m && [2, 3000, +m[1]]
        }
    }
]
...
/**
 * Fetches the WhatsApp Web version currently in use.
 * Running an outdated version is the most common cause of 405 / repeated disconnects.
 */
const fetchLatestBaileysVersion = async (options = {}) => {
    let lastError
    for (const source of VERSION_SOURCES) {
        try {
            const result = await axios_1.default.get(source.url, {
                timeout: 10000,
                ...options,
                responseType: 'text'
            })
            const version = source.parse(result.data)
            if (Array.isArray(version) && version.length === 3 && version.every(n => Number.isInteger(n))) {
                return { version, isLatest: true }
            }
            throw new Error(`could not parse version from ${source.url}`)
        } catch (error) {
            lastError = error
        }
    }
...
/** alias kept for parity with other Baileys forks */
const fetchLatestWaWebVersion = fetchLatestBaileysVersion
